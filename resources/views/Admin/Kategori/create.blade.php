...
@section('content')
    ...
    <div class="mt-6">
        ...
        <form action="{{ route('kategori.store') }}" method="post" class="mt-6 bg-white p-6 rounded-lg shadow-lg">
            @csrf
            <div class="mb-4">
                <label for="name_kategori" class="block text-gray-700 font-semibold">Nama Kategori</label>
                <input type="text" name="name_kategori" value="{{ old('name_kategori') }}" id="name_kategori" class="w-full p-3 mt-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#5CB8E4]">
                @error('name_kategori')
