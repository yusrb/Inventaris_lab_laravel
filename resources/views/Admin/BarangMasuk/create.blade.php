...
@section('content')
    ...
    <a href="{{ route('barang_masuk.index') }}" class="text-[#5CB8E4] hover:text-[#3289B7] underline ml-1 relative top-[7px]">
        Kembali ke Daftar Barang Masuk
    </a>

    <div class="mt-6 bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-semibold mb-3 ml-2">Keranjang Barang Masuk</h2>
        ...
        @else
            <table class="w-full border-collapse border border-gray-300 mt-4 shadow-md">
                <thead>
                    <tr class="bg-[#5CB8E4] text-white text-left">
                        ...
                        <th class="border border-gray-300 p-3">Supplier</th>
                        <th class="border border-gray-300 p-3">Jumlah</th>
                        <th class="border border-gray-300 p-3">Tanggal Masuk</th>
                ...
                <tbody>
                    @foreach($cart as $index => $item)
                        <tr class="bg-white transition">
                            ...
                            <td class="border border-gray-300 p-3">{{ $item['name_supplier'] }}</td>
                            <td class="border border-gray-300 p-3">{{ $item['jumlah'] }}</td>
                            <td class="border border-gray-300 p-3">{{ $item['tanggal_masuk'] }}</td>
                            <td class="border border-gray-300 p-3">{{ $item['keterangan'] }}</td>
                            <td class="border border-gray-300 p-3">
                                <form action="{{ route('barang_masuk.removeFromCart', $index) }}" method="post">
                                    @csrf
            ...
            <div class="mt-4 flex gap-3">
                <form action="{{ route('barang_masuk.store') }}" method="post" class="flex-grow">
                    @csrf
    ...
    <div class="mt-6 bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-semibold mb-3">Tambah Barang Masuk</h2>

        <form action="{{ route('barang_masuk.addToCart') }}" method="post">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal_masuk" id="tanggal_masuk" class="w-full p-3 mt-2 border border-gray-300 rounded-lg">
            </div>
