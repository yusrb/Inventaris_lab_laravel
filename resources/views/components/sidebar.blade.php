    ...
    <nav class="flex-grow mt-6">
        <ul class="space-y-1">
            ...
            @if(auth()->user()->role == 0)
                ...
                @if(Route::has('log_aktivitas.index'))
                    <li>
                        <a href="{{ route('log_aktivitas.index') }}" class="flex items-center gap-3 px-4 py-2 {{ Route::currentRouteName() == 'log_aktivitas.index' ? 'bg-[#5CB8E4]' : '' }} rounded-lg hover:bg-[#3289B7] transition">
